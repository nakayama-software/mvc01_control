...
public:
    CmdVelToBLVR() : Node("cmd_vel_to_blvr")
    {
        ...
        // scaling & limits
        lin_scale_  = this->declare_parameter<double>("linear_scale", 1.0);   // multiplier on incoming Twist.linear.x
        ang_scale_  = this->declare_parameter<double>("angular_scale", 1.0);  // multiplier on incoming Twist.angular.z
        vx_limit_   = this->declare_parameter<double>("vx_limit", 0.5);       // m/s
        w_limit_    = this->declare_parameter<double>("w_limit", 1.68);       // rad/s
        ...
        // (cmd/fbk speed registers in controller; if berbeda, override)
        ...
        // Set mode Vx-ω (address per perangkatmu)
        mb_->with_retry([&]{ write_mode_vx_w(); return 0; });
        ...
        cmd_sub_ = this->create_subscription<geometry_msgs::msg::Twist>(
            ...
            std::bind(&CmdVelToBLVR::cmd_cb, this, _1));

        // Estop opsional (latched true sampai cmd valid & estop false)
        estop_sub_ = this->create_subscription<std_msgs::msg::Bool>(
            estop_topic_, rclcpp::QoS(10).best_effort(),
            std::bind(&CmdVelToBLVR::estop_cb, this, _1));

        // Publishers (telemetry sama seperti versi joystick)
...
private:
    // Command registers (sesuaikan dengan perangkatmu)
    ...
    void write_mode_vx_w(){
        mb_->write_u32(ADDR_MODE, 1); // Vx-ω mode
    }
    ...
    // Poll loop (telemetry sama seperti punyamu, sedikit dirapikan)
    void poll_loop(){
        try{
            auto stamp = this->now();
            // Wheel odom block
            ...
            // Feedback speeds (example block @2492..)
            {
                uint16_t fb[6];
                mb_->with_retry([&]{ mb_->read_block(2492, 6, fb); return 0; });
            ...
            {
                uint16_t r[4];
                mb_->with_retry([&]{ mb_->read_block(addr_disp_fb_, 4, r); return 0; });
            ...
            if (addr_pwr_cnt_ > 0){
                ...
                c.data = mb_->with_retry([&]{ return mb_->read_i32(addr_pwr_cnt_); });
    ...
    double read_scaled(int addr, double s){
        int32_t v = mb_->with_retry([&]{ return mb_->read_i32(addr); });
    ...
    double timeout_s_{2.0}, cmd_rate_hz_{50.0}, poll_rate_hz_{20.0}, cmd_timeout_s_{0.5};
...
int main(int argc, char **argv){
    ...
    rclcpp::spin(std::make_shared<CmdVelToBLVR>());
    rclcpp::shutdown();
    return 0;
}